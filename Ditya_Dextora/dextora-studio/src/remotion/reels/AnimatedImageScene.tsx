...
import { AbsoluteFill, Sequence, useVideoConfig, interpolate, spring, useCurrentFrame, Img, Video, staticFile, Audio } from "remotion";
...
export const AnimatedImageScene: React.FC<{ segment: ReelSegment; index: number }> = ({ segment, index }) => {
    ...
    if (!videoSource && (!segment.imageUrls || segment.imageUrls.length === 0)) {
        return (
            <AbsoluteFill style={{ backgroundColor: "black", justifyContent: "center", alignItems: "center", color: "white" }}>
                <div style={{ fontSize: "2rem" }}>Generating visuals...</div>
            </AbsoluteFill>
        );
    }
